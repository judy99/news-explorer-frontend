...
function SavedNewsHeader (props) {
  ...
  function keywordArrayToShow () {
    if (props.keywordArray.length > NUM_KEYWORDS) {
      const a = props.keywordArray;
      console.log('keyword array: ', props.keywordArray);
  ...
  return (
    <div className='articles-page__info'>
      ...
      <h2 className='heading articles-page__heading'>{currentUser ? currentUser.username : ''}, you have {props.savedArticles.length} saved articles</h2>
      <div className='articles-page__keyword'>
        <span className='articles-page__keywords-by'>By keywords:&nbsp;</span>
        <span className='articles-page__keywords'>{props.keywordArray}, and {otherKeywordNums} other</span>
      </div>
    </div>
  );
}
